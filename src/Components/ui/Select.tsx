...
export default function Select() {
  const [selectedId, setSelectedId] = useState(Categories[3].id);

  const selectedPerson = Categories.find((p) => p.id === selectedId);
      ...
      <div className="relative mt-2">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-xl bg-stone-300 p-2  text-left text-amber-700">
          <span className="col-start-1 row-start-1 flex items-center gap-3 pr-6">
            <img alt="" src={selectedPerson?.imageURL} className="size-5 shrink-0 rounded-full" />
